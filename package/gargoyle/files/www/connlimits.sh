...
<fieldset>
	<legend class="sectionheader">连接数限制</legend>
	<div>
		<label class='narrowleftcolumn' for='max_connections' id='max_connections_label'>最大连接数:</label>
		<input type='text' class='rightcolumn' onkeyup='proofreadNumericRange(this,1,16384)' id='max_connections' size='10' maxlength='5' />
		<em>(最大 16384)</em>
	</div>
	<div>
		<label class='narrowleftcolumn' for='tcp_timeout' id='tcp_timeout_label'>TCP 超时:</label>
		<input type='text' class='rightcolumn' onkeyup='proofreadNumericRange(this,1,3600)' id='tcp_timeout' size='10' maxlength='4' />
		<em>秒 (最大 3600)</em>
	</div>
	<div>
		<label class='narrowleftcolumn' for='udp_timeout' id='udp_timeout_label'>UDP 超时:</label>
